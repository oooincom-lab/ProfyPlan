...
PARAMS: list[dict] = [
    ...
    # Приоритетные заказы (режим жёсткого планирования приоритетного заказа)
    {"key": "priority_orders.enabled", "group": "Приоритетные заказы", "type": "bool", "default": False,
     "title": "Использование приоритетных заказов",
     "hint": "Включает режим приоритетных заказов: жёсткий якорь старта, вытеснение других заказов и разрезание. Пока выключено, приоритет заказа влияет только на очерёдность."},
    {"key": "priority_orders.allow_start_before_predecessors", "group": "Приоритетные заказы", "type": "bool", "default": True,
     "title": "Разрешать старт раньше предшественников",
     "hint": "Приоритетный заказ может стартовать раньше, чем готовы его предшественники — последовательность маршрута будет помечена как нарушенная. Если выключить, такие случаи блокируют расчёт."},
    {"key": "priority_orders.on_overload", "group": "Приоритетные заказы", "type": "enum", "default": "warn",
     "options": ["warn", "block"],
     "title": "При перегрузке ресурса",
     "hint": "«Считать с предупреждением» — строим план и показываем перегрузку. «Запрещать расчёт» — отказываемся считать, пока конфликт не разрешён."},
    {"key": "priority_orders.on_zero_capacity", "group": "Приоритетные заказы", "type": "enum", "default": "warn",
     "options": ["warn", "block"],
     "title": "При нулевой мощности в окне",
     "hint": "Что делать, если приоритетная операция попадает в окно, где мощность ресурса нулевая (простой или ремонт)."},
    {"key": "priority_orders.resource_priority", "group": "Приоритетные заказы", "type": "bool", "default": False,
     "title": "Приоритет по общим ресурсам",
     "hint": "Операции приоритетного заказа выполняются первыми, операции других заказов на том же ресурсе откладываются."},
    {"key": "priority_orders.compete_by_priority", "group": "Приоритетные заказы", "type": "bool", "default": False,
     "title": "Конкуренция приоритетных заказов по приоритетам",
     "hint": "Выключено — все приоритетные равны, работает правило «кто раньше начал». Включено — приоритетные конкурируют по лестнице приоритетов; при равенстве — кто раньше начал."},
    {"key": "priority_orders.allow_preemption", "group": "Приоритетные заказы", "type": "bool", "default": False,
     "title": "Разрешать прерывание (разрезание) операций",
     "hint": "Позволяет делить операцию между приоритетным и обычным заказом на одном ресурсе. Выключено — чужие операции сдвигаются целиком."},
    {"key": "priority_orders.min_chunk", "group": "Приоритетные заказы", "type": "enum", "default": "half_shift",
     "options": ["hour", "half_shift", "shift", "day"],
     "title": "Минимальный непрерывный участок",
     "hint": "Наименьший кусок, на который можно разрезать операцию. Меньше — точнее план, но ресурс чаще переключается."},
    {"key": "priority_orders.max_preemptions", "group": "Приоритетные заказы", "type": "number", "default": 2,
     "title": "Лимит прерываний на операцию",
     "hint": "Сколько раз одну операцию можно прервать. Больше двух — смысл операции теряется, график становится нечитаемым."},
    {"key": "priority_orders.allow_interleave", "group": "Приоритетные заказы", "type": "bool", "default": False,
     "title": "Разрешать частичное переплетение",
     "hint": "Позволяет двум приоритетным заказам делить один ресурс по сменам внутри окна, когда сдвиг якоря невозможен."},
    {"key": "priority_orders.interleave_step", "group": "Приоритетные заказы", "type": "enum", "default": "shift",
     "options": ["half_shift", "shift", "day"],
     "title": "Шаг чередования при переплетении",
     "hint": "Как часто операции меняются на ресурсе: каждые полсмены, смену или день."},
    {"key": "priority_orders.interleave_max_operations", "group": "Приоритетные заказы", "type": "number", "default": 2,
     "title": "Лимит участников переплетения",
     "hint": "Сколько операций может делить один ресурс. Больше 2–3 — ресурс превращается в лоскутное одеяло."},
    {"key": "priority_orders.shift_dependents", "group": "Приоритетные заказы", "type": "bool", "default": True,
     "title": "Сдвигать зависимые заказы",
     "hint": "Автоматически переносить заказы-потребители и связанные заказы при сдвиге якоря."},
    {"key": "priority_orders.keep_before_after", "group": "Приоритетные заказы", "type": "bool", "default": True,
     "title": "Хранить план до/после и журнал сдвигов",
     "hint": "Нужно для отката и для показа «призрака» — что именно изменилось из-за приоритетного заказа."},
    # Привычка интерфейса, а не политика плана — поэтому живёт в группе «Интерфейс».
    ...
    {"key": "priority_orders.inherit_priority_ask", "group": "Интерфейс", "type": "bool", "default": True,
     "title": "Спрашивать про наследование приоритета",
     "hint": "При изменении приоритета у родительского заказа показывать вопрос: применить ли новый приоритет к дочерним заказам. Это привычка интерфейса, а не политика плана."},
    {"key": "priority_orders.recommended_level", "group": "Интерфейс", "type": "enum", "default": "project",
     "options": ["project", "cluster", "workspace"],
     "title": "Где задавать политику приоритетных заказов",
     "hint": "Проект — рамка политики, кластер — правила конкуренции за ресурс, рабочий стол — если политика общая для всех проектов."},
    {"key": "priority_orders.chain_integrity", "group": "Приоритетные заказы", "type": "enum", "default": "whole",
     "options": ["whole", "warn", "off"],
     "title": "Целостность цепочки заказов",
     "hint": "Цепочка заказов (передел) входит в кластер или группу целиком: «целиком» — жёстко, «предупредить» — с вопросом, «выключено» — как раньше. Разрыв цепочки ломает общий граф кластера."},
    {"key": "priority_orders.free_order_as_pool", "group": "Приоритетные заказы", "type": "bool", "default": True,
     "title": "Свободный заказ — кластер из одного",
     "hint": "Включено — у одиночного заказа тоже есть общий граф и общие ресурсы, он участвует в выравнивании. Выключено — не участвует."},
]
