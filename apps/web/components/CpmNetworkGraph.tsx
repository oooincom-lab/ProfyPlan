'use client';
/* Граф CPM — утверждённая версия (Вариант 2: дни на связях).
   Собран из страницы /cpm (удалена 12.09.2026) одним непрерывным куском,
   чтобы не нарушить структуру разметки. Данные приходят параметрами. */
import React, { useState, useEffect, useRef, useCallback } from 'react';
import {
  spreadLayersVertical, buildAbsoluteLayout, buildRelativeLayout, cloneLayout,
  toDays, fmtDur, getNodeR, addDays, fmt, isWeekend, isHoliday,
  UNITS, HOLIDAYS_2026, PROJECT_START,
} from '@/lib/cpmNetwork';


class GraphErrorBoundary extends React.Component<{ children?: any }, { err: string | null }> {
  constructor(props: any) { super(props); this.state = { err: null }; }
  static getDerivedStateFromError(e: any) { const st = String((e && e.stack) || '').split('\n').slice(0, 3).join(' | '); return { err: String((e && (e.message || e)) || e) + (st ? ' ⇢ ' + st : '') }; }
  componentDidCatch(e: any) { console.error('[CPM] render error:', e); }
  render() {
    if (this.state.err) {
      return (
        <div style={{ padding: 16, color: '#F87171', fontSize: 13, whiteSpace: 'pre-wrap' }}>
          Ошибка графа: {this.state.err}
        </div>
      );
    }
...
const graphStubs: any = {
  getBOMTree: async () => null, explodeAndSaveBOM: async () => null, uploadBOM: async () => null,
  getProjects: async () => [], runCPM: async () => null, login: async () => null, demo: null,
};
const ExcelImportWizard: any = () => null;
const GanttChart: any = () => null;
...
export default function CpmNetworkGraph({ cpmResult, projectName, onSelect }: CpmGraphProps) {
  // данные текущего расчёта проекта → операции и связи графа
  const ops: any[] = ((cpmResult && cpmResult.nodes) || []).map((n: any) => ({
    id: String(n.id),
    name: n.name || String(n.id),
    dur: (n.duration_days != null ? n.duration_days : (n.duration_base != null ? n.duration_base : 1)),
    unit: 'd',
    crit: !!n.critical,
    float: n.total_float_days || 0,
  }));
  const _edges: any[] = (cpmResult && (cpmResult.edges || cpmResult.links)) || [];
  const deps: [string, string][] = _edges
    .map((e: any) => [String(e.from ?? e.source ?? e.predecessor_id ?? e.predecessor),
  ...
  const [sprMode, setSprMode] = useState(1); // 0=╤Б╨┤╨╡╤А╨╢, 1=╨╛╨┐╤В╨╕╨╝, 2=╨░╨│╤А╨╡╤Б
  ...
  // Init demo
  useEffect(()=>{
    setupLayout(graphStubs.demo,graphStubs.demo);
    graphStubs.getProjects().then((res:any)=>{
  ...
  const draw = useCallback(()=>{
    ...
    if(viewMode==='relative'){
      var totalDays=Math.ceil(ops.reduce(function(m: any, o: any){return Math.max(m,o.ef);},0));
    ...
    for(var i=0;i<vis.length;i++){
      ...
      // Number тАФ centered, no duration inside
      ...
      // Reserve below тАФ only for non-critical
      if(!o.crit&&o.tf>0){
  ...
  var changeUnit=useCallback((v:number)=>{
    var names=['m','h','d'];var labels=['╨╝╨╕╨╜','╤З╨░╤Б','╨┤╨╜╨╕'];
  ...
  // тФАтФА BOM handlers тФАтФА
  var loadBOMTree=useCallback(async (pid:string)=>{
    setBomLoading(true);setBomMsg(null);
    try{
      var tree=await graphStubs.getBOMTree(pid) as any;
      setBomNodes(tree.nodes||[]);
      if((tree.nodes||[]).length===0)setBomMsg('BOM-╨┤╨╡╤А╨╡╨▓╨╛ ╨┐╤Г╤Б╤В╨╛.');
    }catch(e:any){setBomMsg('╨Ю╤И╨╕╨▒╨║╨░: '+e.message);setBomNodes([]);}
  ...
  var handleBOMUpload=useCallback(async (pid:string,file:File)=>{
    setBomLoading(true);setBomMsg(null);
    try{
      var result=await graphStubs.uploadBOM(pid,file);
      setBomMsg('╨Ч╨░╨│╤А╤Г╨╢╨╡╨╜╨╛: '+result.imported+' ╤Г╨╖╨╗╨╛╨▓.'+(result.errors.length>0?' ╨Ю╤И╨╕╨▒╨║╨╕: '+result.errors.join('; '):''));
      if(result.imported>0)await loadBOMTree(pid);
    }catch(e:any){setBomMsg('╨Ю╤И╨╕╨▒╨║╨░ ╨╖╨░╨│╤А╤Г╨╖╨║╨╕: '+e.message);}
    finally{setBomLoading(false);}
  },[]);
  ...
  var handleExplodeAndSave=useCallback(async (pid:string)=>{
    ...
    try{
      var r=await graphStubs.explodeAndSaveBOM(pid);
      setBomMsg('╨б╨╛╨╖╨┤╨░╨╜╨╛ '+r.created_operations+' ╨╛╨┐╨╡╤А╨░╤Ж╨╕╨╣, '+r.created_dependencies+' ╤Б╨▓╤П╨╖╨╡╨╣.'+(r.warnings.length>0?' тЪа '+r.warnings.length+' ╨┐╤А╨╡╨┤╤Г╨┐╤А╨╡╨╢╨┤.'+r.warnings.slice(0,2).join(' | '):''));
      // Reload CPM after explosion
      await loadProject(pid);
    }catch(e:any){setBomMsg('╨Ю╤И╨╕╨▒╨║╨░: '+e.message);}
  ...
  var onMouseMove=useCallback((e:React.MouseEvent)=>{
    ...
    if(found&&found.id!==hoverIdRef.current){
      ...
      tip.innerHTML='<div class="tooltip-title">\u2116'+found.num+' \u00b7 '+found.name+'</div>'
        +'<div class="tooltip-row">╨Ф╨╗╨╕╤В╨╡╨╗╤М╨╜╨╛╤Б╤В╤М <b>'+fmtDur(found.dur,found.unit,displayUnitRef.current)+'</b></div>'
        +'<div class="tooltip-row">ES <b>╨┤╨╡╨╜╤М '+found.es+'</b></div><div class="tooltip-row">EF <b>╨┤╨╡╨╜╤М '+found.ef+'</b></div>'
        +'<div class="tooltip-row">LS <b>╨┤╨╡╨╜╤М '+found.ls+'</b></div><div class="tooltip-row">LF <b>╨┤╨╡╨╜╤М '+found.lf+'</b></div>'
        +'<div class="tooltip-row">╨Я╨╛╨╗╨╜╤Л╨╣ ╤А╨╡╨╖╨╡╤А╨▓ <b style="color:'+(found.crit?'var(--danger)':'var(--success)')+'">'+found.tf+' ╨┤╨╜.</b></div>'
        +'<div class="tooltip-row" style="margin-top:4px;color:'+(found.crit?'var(--danger)':'var(--fg-4)')+';">'+(found.crit?'тЪа ╨Ъ╤А╨╕╤В╨╕╤З╨╡╤Б╨║╨╕╨╣ ╨┐╤Г╤В╤М':'╨б ╤А╨╡╨╖╨╡╤А╨▓╨╛╨╝')+'</div>';
  ...
  var loadProject=useCallback(async (pid:string)=>{
    ...
    try{
      await graphStubs.login(graphStubs.demo, graphStubs.demo);
      var cpm=await graphStubs.runCPM(pid);
      console.log('[CPM] API response:',cpm);
      var nodes=cpm.nodes||[];
      var edges=(cpm as any).edges||(cpm as any).dependencies||[];
      if(!nodes.length)throw new Error('╨Э╨╡╤В ╨╛╨┐╨╡╤А╨░╤Ж╨╕╨╣ ╨▓ ╨┐╤А╨╛╨╡╨║╤В╨╡');
      // Map API nodes
      var apiOps:Op[]=nodes.map(function(n:any,i:number){
        ...
        return {
          ...
          name:n.name||'╨Ю╨┐╨╡╤А╨░╤Ж╨╕╤П '+(i+1),dur:dur,unit:unit,
      ...
      apiOps.unshift({id:'S',num:'S',name:'╨б╤В╨░╤А╤В',dur:0,unit:'d',es:0,ef:0,ls:0,lf:0,tf:0,crit:1});
      apiOps.push({id:'F',num:'F',name:'╨д╨╕╨╜╨╕╤И',dur:0,unit:'d',es:totalEf,ef:totalEf,ls:totalEf,lf:totalEf,tf:0,crit:1});
      // Map edges
      var apiDeps:[string,string][]=edges.map(function(e:any){return [e.from||e.predecessor_id||e.source,e.to||e.successor_id||e.target];});
      if(!apiDeps.length){
        // Fallback: link SтЖТfirst, FтЖРlast, and linear chain
      ...
      setLabel((cpm as any).project_name||'╨Я╤А╨╛╨╡╨║╤В'+' ┬╖ CPM');
  ...
  const loadGanttData = useCallback(async (pid: string) => {
    return; // своя загрузка отключена: данные приходят из расчёта проекта
    try {
  ...
  // раскладка из переданных данных расчёта
  useEffect(() => {
    try {
      // ссылки, которыми пользуются отрисовка и обработчики
      (opsRef as any).current = ops;
      (depsRef as any).current = deps;
      setupLayout(ops, deps);
      draw();
    } catch (e: any) {
      console.error('[CPM] layout error', e);
    }
  }, [cpmResult]);
    ...
    <div style={{fontFamily:"'Inter',system-ui,sans-serif",background:'var(--bg)',color:'var(--fg)',overflow:'hidden',height:'100vh'}}>
      ...
      <div className="topbar">
        ...
        <div className="mode-tabs">
          {(['absolute','relative'] as Mode[]).map(m=>(
            <button key={m} onClick={()=>switchMode(m)} className={'mode-btn'+(viewMode===m?' active':'')}>{m==='absolute'?'╨Р╨▒╤Б╨╛╨╗╤О╤В╨╜╤Л╨╣':'╨Ю╤В╨╜╨╛╤Б╨╕╤В╨╡╨╗╤М╨╜╤Л╨╣'}</button>
          ))}
        </div>
        <div className="legend">
          <div className="unit-slider">
            <span className="unit-label" style={{fontSize:10,color:'var(--fg-4)'}}>╨Ь╨░╤Б╤И╤В╨░╨▒:</span>
            <span className="unit-label">╨╝╨╕╨╜</span>
            <input type="range" min={0} max={2} value={['m','h','d'].indexOf(displayUnit)} step={1}
              onChange={e=>changeUnit(parseInt(e.target.value))}/>
            <span className="unit-label">╨┤╨╜╨╕</span>
            <span className="unit-label" style={{color:'var(--accent-3)',fontWeight:700}}>{{m:'╨╝╨╕╨╜',h:'╤З╨░╤Б',d:'╨┤╨╜╨╕'}[displayUnit]}</span>
          </div>
          <div className="unit-slider">
            <span className="unit-label" style={{fontSize:10,color:'var(--fg-4)'}}>╨а╨░╨╖╨▒╤А╨╛╤Б:</span>
            <span className="unit-label">╤Б╨┤╨╡╤А╨╢</span>
            <input type="range" min={0} max={2} value={sprMode} step={1}
              onChange={e=>changeSpr(parseInt(e.target.value))}/>
            <span className="unit-label">╨░╨│╤А╨╡╤Б</span>
            <span className="unit-label" style={{color:'var(--accent-3)',fontWeight:700}}>{['╤Б╨┤╨╡╤А╨╢','╨╛╨┐╤В╨╕╨╝','╨░╨│╤А╨╡╤Б'][sprMode]}</span>
          </div>
          <div className="unit-slider">
            <span className="unit-label" style={{fontSize:10,color:'var(--fg-4)'}}>╨и╤А╨╕╤Д╤В:</span>
            <span className="unit-label">╤Б╤В╨░╨╜╨┤</span>
            <input type="range" min={0} max={1} value={fontScale===1.5?1:0} step={1}
              onChange={e=>changeFont(parseInt(e.target.value))}/>
            <span className="unit-label">╨║╤А╤Г╨┐╨╜</span>
            <span className="unit-label" style={{color:'var(--accent-3)',fontWeight:700}}>{fontScale===1.5?'╨║╤А╤Г╨┐╨╜':'╤Б╤В╨░╨╜╨┤'}</span>
          </div>
          <div className="legend-item"><div className="legend-dot" style={{borderColor:'var(--danger)',background:'rgba(239,68,68,0.18)',boxShadow:'0 0 8px rgba(239,68,68,0.5)'}}/> ╨Ъ╤А╨╕╤В╨╕╤З╨╡╤Б╨║╨╕╨╣</div>
          <div className="legend-item"><div className="legend-dot" style={{borderColor:'var(--accent-3)',background:'rgba(59,130,246,0.08)',boxShadow:'0 0 8px rgba(59,130,246,0.35)'}}/> ╨б ╤А╨╡╨╖╨╡╤А╨▓╨╛╨╝</div>
          <button onClick={resetView} className="btn">╨б╨▒╤А╨╛╤Б</button>
          <button onClick={toggleCritOnly} className={'btn'+(critOnly?' btn-primary':'')}>╨в╨╛╨╗╤М╨║╨╛ ╨║╤А╨╕╤В. ╨┐╤Г╤В╤М</button>
          <button onClick={saveDefaults} className="btn" title="╨б╨╛╤Е╤А╨░╨╜╨╕╤В╤М ╤В╨╡╨║╤Г╤Й╨╕╨╡ ╨╜╨░╤Б╤В╤А╨╛╨╣╨║╨╕ ╨┐╨╛ ╤Г╨╝╨╛╨╗╤З╨░╨╜╨╕╤О" style={{padding:'4px 8px',fontSize:16,lineHeight:1}}>тЪЩ</button>
        </div>
      </div>
        ...
        <div style={{display:'flex',alignItems:'stretch',gap:0}}>
          ...
          <div style={{display:'flex',gap:8,alignItems:'center',padding:'6px 10px',background:'var(--bg-2)',border:'1px solid var(--border)',borderTop:'none',borderRadius:'0 0 0 8px'}}>
            ...
            <button onClick={()=>{if(selectedId)setShowExcelWizard(true);}}
              className="btn"
              disabled={!selectedId}
              title="╨Ш╨╝╨┐╨╛╤А╤В Excel (╨Ч╨░╨║╨░╨╖╤Л + BOM + ╨Ь╨░╤А╤И╤А╤Г╤В╤Л)"
            ...
            {loading&&<span style={{fontSize:11,color:'var(--accent-3)'}}>╨Ч╨░╨│╤А╤Г╨╖╨║╨░...</span>}
            {error&&(
              <button onClick={()=>setError(null)} style={{fontSize:11,color:'var(--danger)',background:'none',border:'none',cursor:'pointer'}}>{error} тЬХ</button>
            )}
          </div>
          {/* BOM panel */}
          {bomPanel && selectedId && (
            <div style={{
              ...
              {bomLoading ? <div style={{fontSize:10,color:'var(--fg-4)',padding:'8px 0'}}>╨Ч╨░╨│╤А╤Г╨╖╨║╨░...</div> :
              (bomNodes||[]).length===0 ? <div style={{fontSize:10,color:'var(--fg-4)',padding:'8px 0'}}>
                {bomMsg||'╨Э╨╡╤В ╨┤╨░╨╜╨╜╤Л╤Е. ╨Ч╨░╨│╤А╤Г╨╖╨╕╤В╨╡ BOM-╤Д╨░╨╣╨╗.'}
              </div> :
                ...
                {(bomNodes||[]).map(n=>(
                  <div key={n.id} style={{
                    ...
                    <span style={{fontSize:9,color:n.is_make_or_buy==='make'?'var(--accent-3)':'var(--warning)',flexShrink:0}}>
                      {n.is_make_or_buy==='make'?'ЁЯФз':'ЁЯУж'}
                    </span>
                    <span style={{flexShrink:0,fontWeight:600,color:'var(--fg-4)'}}>{(n.path||'').split('.').pop()||'┬╖'}</span>
                    <span title={n.nomenclature_name}>{n.nomenclature_name.length>28?n.nomenclature_name.slice(0,26)+'тАж':n.nomenclature_name}</span>
                    <span style={{marginLeft:'auto',fontSize:9,color:'var(--fg-4)',flexShrink:0}}>
                      ├Ч{n.quantity_per_parent} {n.unit}
                    </span>
                  </div>
                ))}
              </div>}
              ...
              <div style={{display:'flex',gap:6,marginTop:4}}>
                ...
                <button className="btn btn-primary" disabled={bomExploding||(bomNodes||[]).length===0}
                  ...
                  {bomExploding?'тП│ ╨У╨╡╨╜╨╡╤А╨░╤Ж╨╕╤П...':'тЪб BOM тЖТ ╨Я╤А╨╛╤Ж╨╡╤Б╤Б╤Л'}
                </button>
              </div>
              {bomMsg && (
                <div style={{marginTop:6,fontSize:9,color:bomMsg.startsWith('╨б╨╛╨╖╨┤╨░╨╜╨╛')||bomMsg.startsWith('╨Ч╨░╨│╤А╤Г╨╢╨╡╨╜╨╛')?'var(--success)':'var(--warning)',lineHeight:1.4}}>
                  {bomMsg}
                </div>
              )}
            </div>
      ...
      <div ref={wrapRef} className={'canvas-wrap'+(canvasState?' '+canvasState:'')}
        ...
        <div className="info-panel">
          <strong>╨Ъ╤А╨╕╤В╨╕╤З╨╡╤Б╨║╨╕╨╣ ╨┐╤Г╤В╤М:</strong> {cpDur} ╨┤╨╜. ┬╖ {cpCount} ╨╛╨┐╨╡╤А╨░╤Ж╨╕╨╣<br/>
          <span style={{color:'var(--fg-4)',marginTop:2,display:'inline-block'}}>
            Drag ┬╖ Scroll ┬╖ ╨а╨╡╨╢╨╕╨╝: {viewMode==='absolute'?'╨Р╨▒╤Б╨╛╨╗╤О╤В╨╜╤Л╨╣':'╨Ю╤В╨╜╨╛╤Б╨╕╤В╨╡╨╗╤М╨╜╤Л╨╣'}
          </span>
        </div>
        <div className="scale-control">
          ...
          <button onClick={()=>{scaleRef.current=Math.max(0.15,scaleRef.current/1.15);setScalePercent(Math.round(scaleRef.current*100));draw();}}>тИТ</button>
        </div>
      </div>
...
export function CpmNetworkGraphSafe(props: CpmGraphProps) {
  return (
    <GraphErrorBoundary>
      <CpmNetworkGraph {...props} />
    </GraphErrorBoundary>
  );
}
