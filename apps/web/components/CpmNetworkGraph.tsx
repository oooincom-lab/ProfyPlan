...
/* Граф CPM — утверждённая версия (Вариант 2: дни на связях).
   Собран из страницы /cpm, удалённой 12.09.2026: раскладка, отрисовка на канве,
   масштаб/панорама, выбор операции. Данные приходят параметрами. */
...
const graphStubs: any = {
  ...
  login: async () => null, runCPM: async () => null, getProjects: async () => [],
  projects: [], demo: null,
};
...
export default function CpmNetworkGraph({ cpmResult, projectName, onSelect }: CpmGraphProps) {
  ...
  // Mutable refs
  const opsRef = useRef(graphStubs.demo);
  const depsRef = useRef(graphStubs.demo);
