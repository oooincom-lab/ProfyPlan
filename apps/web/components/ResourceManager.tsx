...
export default function ResourceManager({ projects, windowMode = false, debug = false, onOpenResEdit, onOpenDirPick, onEditItem, onOpenWsched }: { projects: Project[]; windowMode?: boolean; debug?: boolean; onOpenResEdit?: (res: any | null) => void; onOpenDirPick?: (entity: string, onPick: (row: any) => void) => void; onEditItem?: (entity: string, id: string, row: any) => void; onOpenWsched?: () => void }) {
        ...
        {formOpen && (
          <>
            ...
            <div style={{ position: 'fixed', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', width: 640, maxWidth: '92vw', maxHeight: '86vh', overflow: 'auto', zIndex: 9991, background: 'linear-gradient(135deg, #0F1E36, #162844)', border: '1px solid #1E3252', borderRadius: 14, padding: 20, boxShadow: '0 24px 80px rgba(0,0,0,0.6)' }}>
              ...
              <ResourceForm form={form} onChange={patch => setForm({ ...form, ...patch })} schedules={schedules} saving={saving} onSave={save} onCancel={() => { setFormOpen(false); setEditingId(null); setForm({}); }} onOpenDirPick={onOpenDirPick} onEditItem={onEditItem} onOpenWsched={onOpenWsched} />
            </div>
          </>
        )}
