...
**Открытый вопрос по 2.8:** в проверке на паре смежных операций одного ресурса (разрыв задан 30 дней) система отчиталась «сдвинуто 1 операция» и выдала предупреждение о приоритете непрерывности, но дата операции в плане не изменилась, при этом ни ошибки, ни пропуска ограничений не сообщалось. Нужно досмотреть, на каком шаге ограничение потока теряется (кандидаты: перекрытие более поздним проходом расчёта либо несовпадение идентификаторов операций потока с идентификаторами в проходе).

**Осталось в фазе 2:** довести применение потоков до фактического сдвига плана (**2.8**, открытый вопрос выше), **2.9** ползунок «что если» по мощности, затем интерфейсные **8.4** инспектор выделенного объекта (окна сохраняются) и **8.5** версии и сценарии в шапке.
