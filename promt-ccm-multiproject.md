...
## 25. Вопросы без ответа (сводка 13.09.2026)

Вопросы задавались по ходу обсуждения, но остались без ответа. Вынесены отдельно, чтобы не потерялись:
...
11. **CI/CD:** владельцу отправлено пояснение (автоматическая сборка и выкладка по пушу; нужны секреты репозитория: адрес сервера, пользователь, SSH-ключ). Пока деплой ручной.
