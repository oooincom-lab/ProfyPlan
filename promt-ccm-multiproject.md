...
1. **Excel-импорт (7-вкладочный)** — замыкает контур «данные → расчёт → визуализация»
2. **CPM React Flow** — миграция с iframe на живой граф
3. **Ганта** — базовая диаграмма (read-only)
4. **Группы и Пулы** — модель OrderGroup + OrderPool, API, drag-and-drop на временной шкале
5. **Этапы (Phases)** — модель, API, учёт в CPM (под флагом `uses_phases`)
6. **Подразделения (Departments)** — модель, API, учёт в resource-leveling
7. **E2E-тест на VPS** — прогнать реальные данные через импорт → CPM → CCM → resource-leveling
