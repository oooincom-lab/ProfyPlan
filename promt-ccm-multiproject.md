...
4. **Этапы (Phases)** — модель, API, учёт в CPM (под флагом `uses_phases`)
5. **Подразделения (Departments)** — модель, API, учёт в resource-leveling
6. **E2E-тест на VPS** — прогнать реальные данные через импорт → CPM → CCM → resource-leveling
