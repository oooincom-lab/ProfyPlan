...
## 24. Заказы, группы и пулы в интерфейсе (решено 13.09.2026)

**Решение:** заказ — это основная сущность планирования и общий раздел проекта. Группы и пулы — **способы объединения заказов** для совместного расчёта (куст с общими ресурсами), а не самостоятельные разделы. Разводить их по отдельным рубрикам не нужно: это дробит одну и ту же работу и путает.

**Как это выглядит:** в дереве проекта один пункт — **«Заказы»**. Внутри — переключатель представления:
- **Все заказы** — плоский список со всеми действиями (цепочка, ресурсы, состав);
- **По группам** — те же заказы, сгруппированные по кустам; здесь же создание, переименование и удаление группы;
- **По пулам** — представление пулов с перетаскиванием заказов.

То есть группировка становится **режимом просмотра внутри раздела заказов**, а не отдельной страницей. Существующие экраны групп и пулов сохраняют функциональность (создание, состав, поток, перетаскивание), но открываются из раздела заказов, а не отдельными пунктами меню.

**Следствие для навигации:** пункты «Группы» и «Пулы» из дерева проекта убираются, их место — переключатель в «Заказах». Названия: «Заказы · все», «Заказы · по группам», «Заказы · по пулам».

**Куда в плане:** полировка областей (фаза 3) — объединение пунктов и переключатель представлений; при этом вид «Шкала куста» по-прежнему открывается для выбранного куста (группы или пула).
