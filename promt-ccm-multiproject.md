...
**2.8 — состояние:** механизм работает в расчёте, но ответ пока не отражает результат.
- Ограничение потока доходит до расчёта: в замере `min_start` = 289.58 вместо прежних 243.89 дн, признак «кусается» истина; предупреждение `flow_shift` выдаётся; конфликтов с технологией нет.
- Внутренние узлы расчёта сдвигаются: в журнале `AFTER-CONSTR es=289.58`, итоговый узел операции — 08.11.2027 вместо 06.09.2027.
- **Но в ответе API дата этой операции остаётся прежней (06.09.2027).** Значит между расчётом и формированием ответа есть шаг (календарная/ресурсная раскладка), который возвращает операцию на прежнее место и не учитывает ограничение потока. Закрепления этот шаг проходят (проверено: закрепление «начать не раньше 15.12.2027» сдвигает операцию и в ответе).
- Сделано по ходу: единицы границы потока приведены к тем же, что у закреплений (индекс рабочего дня от якоря); защита от конфликта с технологическим порядком (`flow_conflict`); поштучное применение ограничений с предупреждением `constraints_skipped`; в ответе по каждому потоку видно требуемую границу и позицию операции (`constraints`).
- **Следующий шаг:** применить ограничения потока на том же шаге, что и закрепления, в календарной/ресурсной раскладке — либо заставить её уважать уже посчитанный сдвинутый старт.

**Осталось в фазе 2:** **2.9** ползунок «что если» по мощности, затем интерфейсные **8.4** инспектор выделенного объекта (окна сохраняются) и **8.5** версии и сценарии в шапке.
