...
2. **Набор якорей магнита** — включены базовые: конец чужой операции на общем ресурсе; начало свободного окна мощности ресурса; начало следующей операции. Дополнительно доступен якорь «контрольная дата (веха)». Каждый якорь включается/выключается.
3. **Радиус магнита** — 2 дня (настраиваемо, в днях и/или часах; влияет на «липкость»).
4. **Тип закрепления** — **жёсткое** (CPM обязан соблюсти); мягкий режим доступен как опция.
5. **Показывать «цену» перегрузки в ползунке «что если»** — **да** (сокращение срока + снятые конфликты + рост риска перегрузки muri).
6. **Авто-подбор даты и времени закрепления** — **да** (система предлагает ближайшую точку без конфликтов; пользователь может скорректировать).
...
## 19. Ближайшие работы (в плане, на 11.09.2026)

Реализовано и зафиксировано: события мощности (включая абсолютную мощность), единый сервис мощности, длительность в днях/часах/минутах, учёт в Ганте, CPM и CCM, отчёт потерь (с разрезом по подразделениям), журнал событий, доля мощности проекта, квоты подразделений с интерфейсом, полоса мощности, межпроектные конфликты, предложение/применение сдвига, авто-план по конфликтам с приоритетами, загрузка по неделям и по подразделениям, межпроектное выравнивание.

Осталось (следующие волны):
1. **Применение межпроектного выравнивания одним действием** — сейчас предложение возвращается списком, применение — по каждому проекту отдельно.
2. **Интерфейс отчётов загрузки** — вывести загрузку подразделений и межпроектное выравнивание в интерфейс (сейчас доступны через API).
3. **Учёт квот в загрузке подразделений** — ограничение доступного времени квотами ресурсов.
4. **История изменений плана** — журнал сдвигов (кто/когда/почему сдвинул проект).
