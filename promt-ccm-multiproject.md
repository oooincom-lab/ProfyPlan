...
**2.8 подтверждён на проде:** ограничение потока доходит до расчёта (в замере `min_start` 289.58 вместо прежних 243.89 дн) и сдвигает план — операция встала на 08.11.2027 вместо 06.09.2027. Первое «ограничение не сработало» было ошибкой проверки (сопоставление идентификаторов операций шло с учётом регистра). Дополнительно: единицы границы потока приведены к тем же, что у закреплений (индекс рабочего дня от якоря), добавлена защита от конфликта с технологическим порядком (`flow_conflict`) и поштучное применение ограничений с предупреждением `constraints_skipped`.
