...
Реализовано и зафиксировано: события мощности (форсаж/ограничение/простой/ТО/модернизация) с периодом до минут, единый сервис мощности (рабочее окно из графика, длительность в днях/часах/минутах), учёт в Ганте, CPM и CCM, отчёт «потерянные часы по причинам», журнал событий, доля мощности ресурса в проекте, квоты подразделений (с интерфейсом), полоса мощности в Ганте, межпроектные конфликты общих ресурсов, предложение и применение сдвига, загрузка по неделям.

Осталось (следующие волны):
1. **Авто-план сдвигов сразу по нескольким конфликтам** — единое предложение по всем общим ресурсам проекта с учётом приоритетов проектов, а не по одному конфликту.
2. **Отчёт по загрузке подразделений** — спрос и доступное время в разрезе подразделений (по аналогии с загрузкой по неделям), с учётом квот и графиков подразделений.
3. **Абсолютная мощность события** (`capacity_absolute`) — сейчас учитывается только коэффициент (`capacity_multiplier`); нужно поддержать режим «замена на менее мощный ресурс».
4. **Выравнивание по межпроектным конфликтам** — пересчёт нескольких проектов вместе (сейчас выравнивание работает в рамках одного проекта).
5. **Отчёт потерь по подразделениям/проектам** — расширение отчёта «потерянные часы по причинам» разрезом по подразделениям и периодам.
